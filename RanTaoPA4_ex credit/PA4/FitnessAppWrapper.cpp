...
void FitnessAppWrapper::runApp() // will drive the application
{
	...
	do
	{
		...
		this->perform_user_selection(option);
	} while (option != 9); // 9 = exit the program
}

void FitnessAppWrapper::perform_user_selection(const int &option)
{
	bool success = false;
	...
	case 2: // load weekly EP
		this->loadWeeklyExcercisePlan();
		cout << "Diet plans loaded from ExcercisePlan.txt!" << endl;
		break;
	...
	case 4: // store weekly EP
		this->storeWeeklyExcercisePlan();
		cout << "Diet plans stored to ExcercisePlan.txt!" << endl;
		break;
	...
	case 8: // edit daily EP
		...
		if (!success)
		{
			cout << "Could not find the excercise plan specified!" << endl;
		}
		break;
	case 9: // exit
		this->storeWeeklyDietPlan();
...
void FitnessAppWrapper::loadWeeklyExcercisePlan() // this will call the private loadWeeklyPlan() function
{
	// We want to start reading from the very beginning of the "ExcercisePlan.txt" file.
	// We will close the stream associated with the "ExcercisePlan.txt" file if it's open!
	if (this->mFstrEP.is_open()) // is_open () returns true if file open, false otherwise
	{
		this->mFstrEP.close(); // close it so we can start from the beginning of the file
	}
	else // file is not open - open it and read the week of plans from it
	{
		this->mFstrEP.open("exercisePlans.txt"); // opens in input/output mode by default with fstream object
		this->loadWeeklyPlan(this->mFstrEP, this->mEPList);
		this->mFstrEP.close();
	}
}

void FitnessAppWrapper::displayWeeklyExcercisePlan() // this will call the private displayWeeklyPlan() function
{
	this->displayWeeklyPlan(this->mEPList); // display a weekly diet plan to the screen!
}

void FitnessAppWrapper::storeWeeklyExcercisePlan() // this will call the private storeWeeklyPlan() function
{
	// We want to start overwriting from the very beginning of the "ExcercisePlan.txt" file.
	// We will close the stream associated with the "ExcercisePlan.txt" file if it's open!
	if (this->mFstrEP.is_open()) // is_open () returns true if file open, false otherwise
	{
		// Note: since we are working with fstream objects, if the fstream is initially
...
bool FitnessAppWrapper::editExcercisePlan() // will prompt the user for the name of the plan to edit
{
	system("cls");
	this->displayWeeklyExcercisePlan(); // provide the user with the plans
	cout << "Which diet plan would you like to edit (provide plan name)? ";
	// generally, it's more robust to use numerical values for comparisons
	// because it's very easy to mistype a string, but
	// will stick with std::string so we can practice std::string operations
	...
	for (int day = 0; day < 7; ++day)
	{
		...
		{
			success = true;
			this->mEPList[day].editGoal(); // call the ExcercisePlan object's editGoal ()
		}
	}
...
void FitnessAppWrapper::loadDailyPlan(fstream &fileStream, ExcercisePlan &plan)
{
	fileStream >> plan; // extract 1 diet plan from the file through the fileStream -
						// uses overloaded file stream extraction operator -
						// (fstream, ExcercisePlan)
}

void FitnessAppWrapper::loadWeeklyPlan(fstream &fileStream, ExcercisePlan weeklyPlan[])
{
	// Note: in C++ we can declare a variable in the initialization expression.
	// However the variable can only be used inside of the for (). When the for ()
	// is done executing, the day variable is no longer available.
...
void FitnessAppWrapper::displayDailyPlan(const ExcercisePlan &plan)
{
	cout << plan << endl; // write plan to standard display (screen) - 
						  // uses overloaded standard stream insertion operator -
						  // (ostream, ExcercisePlan)
}

void FitnessAppWrapper::displayWeeklyPlan(const ExcercisePlan weeklyPlan[])
{
	// Note: in C++ we can declare a variable in the initialization expression.
	// However the variable can only be used inside of the for (). When the for ()
	// is done executing, the day variable is no longer available.
...
void FitnessAppWrapper::storeDailyPlan(fstream &fileStream, const ExcercisePlan &plan)
{
	fileStream << plan << endl; // write 1 diet plan to the file - 
								// uses overloaded file stream insertion operator -
								// (fstream, ExcercisePlan)
...
void FitnessAppWrapper::storeWeeklyPlan(fstream &fileStream, const ExcercisePlan weeklyPlan[])
{
	// Note: in C++ we can declare a variable in the initialization expression.
	// However the variable can only be used inside of the for (). When the for ()
	// is done executing, the day variable is no longer available.
